...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        self.canvas.cutShape.connect(self.cutShape)
        ...
        cut = action('&Cut', self.cutSubPic,'C', 'cut', u'Cut sub image',enabled=False)
        cutMode = action('&Cut', self.cutSubPic,'C', 'cut', u'Cut sub image',enabled=False)
        ...
        self.actions = struct(save=save, save_format=save_format, saveAs=saveAs, open=open, close=close, resetAll = resetAll,
                              lineColor=color1, create=create, delete=delete, edit=edit, copy=copy,cut=cut,cutMode=cutMode,
                              ...
                              beginner=(), advanced=(),
                              editMenu=(edit, copy, delete,cut,
                                        None, color1),
                              beginnerContext=(create, edit, copy, delete,cut),
                              advancedContext=(createMode, editMode, edit, copy,
                                               delete, shapeLineColor, shapeFillColor),
                              onLoadActive=(
                                  close, create, createMode, editMode,cut,cutMode),
        ...
        self.actions.beginner = (
            open, opendir, changeSavedir, openNextImg, openPrevImg, verify, save, save_format, None, create, cut,copy, delete, None,
    ...
    def cutShape(self,cutpoints):
        basename = os.path.basename(self.filePath)
        text, ok = QInputDialog.getText(self, 'NameDialog',
                                        'Enter new image name:')
        if ok:
            for shape in self.canvas.cutshapes:
                minX=min(shape[0].x(),shape[2].x())
                maxX=max(shape[0].x(),shape[2].x())
                minY=min(shape[1].y(),shape[3].y())
                maxY=max(shape[1].y(),shape[3].y())
                rect = [minX,minY,maxX-minX,maxY-minY]
                cutter = ImageCutter(self.filePath)
                cutter.cut(rect)

                savename = self.dirname + "/" + os.path.splitext(basename)[0] + '_sub_' + text + \
                           os.path.splitext(basename)[1]

                if not os.path.exists(savename):
                    cutter.saveas(savename)
                    print('copy Rect: ', rect, ' Save As -->: ', os.path.basename(savename))
                else:
                    reply = QMessageBox.information(self, "Warning",savename+" has exists,do cover it ? ",QMessageBox.Yes | QMessageBox.No)
            ...
            self.canvas.cutshapes.pop()
            self.canvas.update()
            pass

        self.canvas.cutshapes=[]
        if self.beginner():  # Switch to edit mode.
            self.canvas.setEditing(True)
            self.actions.cut.setEnabled(True)
        else:
            self.actions.cutMode.setEnabled(True)
    ...
    def openNextImg(self, _value=False):
        ...
        if self.filePath is None:
            ...
            currIndex = self.mImgList.index(self.filePath)
            index = currIndex + 1
            while not os.path.exists(self.mImgList[index]):
                if index < len(self.mImgList):
                    index += 1
                else:
                    break
            if os.path.exists(self.mImgList[index]):
                if index < len(self.mImgList):
